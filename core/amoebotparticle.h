...
class AmoebotParticle : public LocalParticle, public RandomNumberGenerator {
 public:
  ...
  // Causes the particle to crash. (i.e. change the state of the particle to a special error state)
  // This is a virtual function with empty implementation and may be overwritten by particle subclasses.
  virtual void crash() {}
 ...
 protected:
  ...
  // Assuming that this particle has a neighbour incident to the specified port label
  // myLabel, the function labelToNbrLabel converts the label myLabel to the
  // corresponding label of the neighbour.
  int labelToNbrLabel(int myLabel) const;
  ...
  // If no such particle exists, then -1 is returned.
  // If ignoreErrorParticles is specified as false, then the function returns -1,
  // if an error particle is encountered while observing the ports in the specified order.
  template<class ParticleType>
