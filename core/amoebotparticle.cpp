...
int AmoebotParticle::labelToNbrLabel(int myLabel) const {
    Q_ASSERT(hasNbrAtLabel(myLabel));

    auto& nbr = nbrAtLabel<AmoebotParticle>(myLabel);
    Node myNode = occupiedNodeIncidentToLabel(myLabel);
    int myDir = labelToDir(myLabel);

    for (int nbrLabel = 0; nbrLabel < (nbr.isContracted() ? 6 : 10); nbrLabel++) {
        if (nbr.nbrNodeReachedViaLabel(nbrLabel) == myNode && nbrDirToDir(nbr, nbr.labelToDir(nbrLabel)) == (myDir + 3) % 6) {
            return nbrLabel;
        }
    }
