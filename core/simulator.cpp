...
void Simulator::step() {
  ...
  system->activate(errorRate, errorRounds);
...
void Simulator::setErrorRate(int percent) {
  errorRate = percent;
  emit errorRateChanged(percent);
}

void Simulator::setErrorRounds(int rounds) {
  errorRounds = rounds;
  emit errorRoundsChanged(rounds);
}

void Simulator::crashParticleAt(Node node) {
  QMutexLocker locker(&system->mutex);
  system->crashParticleAt(node);
}
