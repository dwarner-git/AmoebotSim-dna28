...
class System {
 public:
  // Signatures for functions which activate (or crash) particles. Must be overridden by
  // any system subclasses; see amoebotsystem.h for more detailed documentation.
  virtual void activate(int errorRate, int errorRounds) = 0;
  virtual void activateParticleAt(Node node) = 0;

  // Signatures for functions which crash particles. Must be overridden by
  // any system subclasses; see amoebotsystem.h for more detailed documentation.
  virtual void crashParticleAt(Node node) = 0;
  virtual void nuke() {};
...
bool System::isConnected(const ParticleContainer& particles) {
  ...
  //occupiedNodes.erase(*occupiedNodes.begin()); // remove the first node already
