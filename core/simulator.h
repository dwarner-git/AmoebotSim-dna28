...
class Simulator : public QObject {
 ...
 signals:
  ...
  void errorRateChanged(int percent);
  void errorRoundsChanged(int rounds);
 ...
 public slots:
  // Responds to control flow signals from the GUI and scripts.
  //
  // start, stop, and step are self-explanatory.
  // stepForParticleAt executes one activation for the specific particle at the given node.
  // setStepDuration updates the delay in milliseconds between particle activations.
  // runUntilTermination activates particles repeatedly until the hasTerminated condition is satisfied.
  // setErrorRate updates the percentage of activations that cause the activated particle to crash.
  // setErrorRounds updates the error rounds number, i.e. errorRounds == 5 means that every 5th round crashes occur with the rate specified by errorRate.
  // crashParticleAt causes the specific particle at the given node to crash.
  // nuke lets all particles crash where a crash is permitted.
  ...
  void setErrorRate(int percent);
  void setErrorRounds(int rounds);
  void crashParticleAt(Node node);
  void nuke();
