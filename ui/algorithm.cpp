...
ShapeFormationFaultTolerantAlg::ShapeFormationFaultTolerantAlg() :
  Algorithm("Basic Fault Tolerant Shape Formation", "shapeformationfaulttolerant") {
  addParameter("# Particles", "200");
  addParameter("Hole Prob.", "0.2");
  addParameter("Shape", "h");
}

void ShapeFormationFaultTolerantAlg::instantiate(const int numParticles,
                                    const double holeProb, const QString mode) {
  std::set<QString> set = ShapeFormationFaultTolerantSystem::getAcceptedModes();
  if (numParticles <= 0) {
    emit log("# particles must be > 0", true);
  } else if (holeProb < 0 || holeProb > 1) {
    emit log("holeProb in [0,1] required", true);
    ...
    emit setSystem(std::make_shared<ShapeFormationFaultTolerantSystem>(numParticles,
                                                          holeProb, mode));
  }
}

LeaderElectionFaultTolerantAlg::LeaderElectionFaultTolerantAlg() :
  Algorithm("Basic Fault Tolerant Leader Election", "leaderelectionfaulttolerant") {
  addParameter("# Particles", "100");
  addParameter("Hole Prob.", "0.2");
}

void LeaderElectionFaultTolerantAlg::instantiate(const int numParticles,
                                    const double holeProb) {
  if (numParticles <= 0) {
    emit log("# particles must be > 0", true);
  } else if (holeProb < 0 || holeProb > 1) {
    ...
    emit setSystem(std::make_shared<LeaderElectionFaultTolerantSystem>(numParticles,
                                                          holeProb));
  }
}

ChiralityComparisonAlg::ChiralityComparisonAlg() :
  Algorithm("Chirality Comparison", "chiralitycomparison") {
  addParameter("# Particles", "200");
  addParameter("Hole Prob.", "0.2");
}

void ChiralityComparisonAlg::instantiate(const int numParticles, const double holeProb) {
  if (numParticles <= 0) {
    emit log("# particles must be > 0", true);
  } else if (holeProb < 0 || holeProb > 1) {
    emit log("holeProb in [0,1] required", true);
  } else {
    emit setSystem(std::make_shared<ChiralityComparisonSystem>(numParticles, holeProb));
  }
}
...
AlgorithmList::AlgorithmList() {
  ...
  // Fault tolerant algorithms and related algorithms.
  _algorithms.push_back(new ShapeFormationFaultTolerantAlg());
  _algorithms.push_back(new LeaderElectionFaultTolerantAlg());
  _algorithms.push_back(new ChiralityComparisonAlg());
