...
class VisItem : public GLItem {
 ...
 protected:
  ...
  void drawFlags(const Particle &p);
  void drawParticle(const Particle& p);
  void drawParticleCenter(const Particle& p);
  void drawBorders(const Particle& p);
  void drawBorderPoints(const Particle& p);
  void drawFromParticleTex(int index, const QPointF& pos, double scaling = 1.0);
  void foo(const Particle& p);
  void drawObjects();
