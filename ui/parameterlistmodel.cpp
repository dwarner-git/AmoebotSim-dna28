...
void ParameterListModel::createSystem(QString algName) {
  ...
  } else if (signature == "shapeformationfaulttolerant") {
    dynamic_cast<ShapeFormationFaultTolerantAlg*>(alg)->
        instantiate(params[0].toInt(), params[1].toDouble(), params[2]);
  } else if (signature == "leaderelectionfaulttolerant") {
    dynamic_cast<LeaderElectionFaultTolerantAlg*>(alg)->
        instantiate(params[0].toInt(), params[1].toDouble());
  } else if (signature == "chiralitycomparison") {
    dynamic_cast<ChiralityComparisonAlg*>(alg)->
        instantiate(params[0].toInt(), params[1].toDouble());
  } else {
